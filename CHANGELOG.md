...
- **K2-Horizon 7B (IFM) runs natively.** `mlx-serve pull k2` fetches the 6-bit pack; thinking at all three effort levels, tool calls and the 512k window work. Models whose only chat terminator is declared in `generation_config.json` now stop where they should.
