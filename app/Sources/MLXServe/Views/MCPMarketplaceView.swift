    ...
    private var header: some View {
        HStack {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text("MCP Marketplace").font(.system(size: AppTypeScale.title, weight: .semibold))
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                ForEach(custom.elements, id: \.key) { id, entry in
                    HStack(spacing: 8) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            HStack(spacing: 6) {
                                Text(id).font(.system(size: AppTypeScale.body, weight: .medium))
                        ...
                        if let session = mcpManager.sessions[id] {
                            ...
                            HStack(spacing: 4) {
                                ...
                                Text("error").font(.system(size: AppTypeScale.body, weight: .medium)).foregroundStyle(.red)
                            }
                            .help(err)
                        }
                        Toggle("", isOn: Binding(
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 10) {
                Image(systemName: entry.icon)
                    ...
                    .frame(width: 28)
                VStack(alignment: .leading, spacing: 2) {
                    Text(entry.name).font(.system(size: AppTypeScale.body, weight: .semibold))
        ...
        VStack(alignment: .leading, spacing: 2) {
            HStack(spacing: 4) {
                Text(L10n.text(input.label))
                    .font(.system(size: AppTypeScale.body, weight: .medium))
                if input.required {
                    Text("*").foregroundStyle(.red).font(.system(size: AppTypeScale.body))
                }
            }
            if input.isSecret {
            ...
            if let help = input.helpText {
                Text(L10n.text(help)).font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary)
            }
        }
    }
}
