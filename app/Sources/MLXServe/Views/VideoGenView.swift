...
struct VideoGenView: View {
    ...
    private var promptSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 8) {
                Text("Prompt").font(.system(size: AppTypeScale.body, weight: .semibold))
                Spacer()
    ...
    private var qualitySection: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("Quality").font(.system(size: AppTypeScale.body, weight: .semibold))
        ...
        return VStack(alignment: .leading, spacing: 6) {
            ...
            if effectiveMode != .oneStage,
               let note = model.twoStageCanvasNote(width: effectiveSize.width, height: effectiveSize.height) {
                Text(L10n.text(note)).font(.system(size: AppTypeScale.body)).foregroundStyle(.orange)
            }
        }
    }
    ...
    private var framesSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                Text("Frames").font(.system(size: AppTypeScale.body, weight: .semibold))
                Spacer()
            ...
            if let warn = frameRAMWarning {
                Text(L10n.text(warn)).font(.system(size: AppTypeScale.body)).foregroundStyle(.orange)
            }
            if let advice = model.framesAdvisory(numFrames) {
                Text(L10n.text(advice)).font(.system(size: AppTypeScale.body)).foregroundStyle(.orange)
            }
        }
    }
    ...
    private func keyframeWell(title: String, note: String, url: Binding<URL?>,
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 6) {
                Text(L10n.text(title)).font(.system(size: AppTypeScale.body, weight: .semibold))
                Text(L10n.text(note))
                    .font(.system(size: AppTypeScale.body))
                    .foregroundStyle(.secondary)
                Spacer(minLength: 0)
    ...
    private var referencesSection: some View {
        if model.supportsReferences {
            VStack(alignment: .leading, spacing: 8) {
                HStack(spacing: 6) {
                    Text("References").font(.system(size: AppTypeScale.body, weight: .semibold))
    ...
    private var speechSection: some View {
        ...
        if !model.supportsAudioInput {
            VStack(alignment: .leading, spacing: 6) {
                HStack(spacing: 6) {
                    Text("Sound").font(.system(size: AppTypeScale.body, weight: .semibold))
    ...
    private var audioInputSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 6) {
                Text("Speech & sound").font(.system(size: AppTypeScale.body, weight: .semibold))
    ...
    private var speechComposer: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if case .failed(let msg) = tts.phase {
                Text(msg).font(.system(size: AppTypeScale.body)).foregroundStyle(.orange)
            }
            ...
            if tts.isRunning {
                HStack(spacing: 8) {
                    ...
                    if case .running(_, _, let msg) = tts.phase {
                        Text(msg).font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary)
        ...
        HStack(spacing: 8) {
            ...
            if leadWithDuration, let d = audioDuration {
                Text(String(format: "%.1fs", d))
                    .font(.system(size: AppTypeScale.body).monospacedDigit())
                    .foregroundStyle(.secondary)
                Text("·").font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary)
            }
            ...
            if model.supportsPipelineModes {
                ...
                HStack(alignment: .firstTextBaseline, spacing: 12) {
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text("Mode").font(.system(size: AppTypeScale.body)).fixedSize()
    ...
    private var loraSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            Divider()
            Text("Style LoRAs").font(.system(size: AppTypeScale.body, weight: .semibold))
    ...
    private func loraRow(index: Int, lora: LoraAdapter) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack(spacing: 8) {
                Text("Scale").font(.system(size: AppTypeScale.body))
        ...
        VStack(spacing: 8) {
            ...
            if !payloadFits {
                Text("\(VideoModelPreset.deliveredFrames(perWindow: numFrames, chainWindows: chainWindows)) frames at \(effectiveSize.width) × \(effectiveSize.height) is more than one response can carry. Shorten the clip, drop a window, or use a smaller canvas.")
                    .font(.system(size: AppTypeScale.body)).foregroundStyle(.orange)
            }
