                ...
                VStack(alignment: .leading, spacing: 1) {
                    ...
                    if let sub = subtitle {
                        Text(agent.isBuiltIn ? L10n.text(sub) : sub)
                            .font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary).lineLimit(1)
                    }
                }
                Spacer(minLength: 0)
...
private struct AgentEditor: View {
    ...
    private var symbolBadge: some View {
        Image(systemName: agent.symbol)
            .font(.system(size: AppTypeScale.heading, weight: .medium))
        ...
        Button {
            ...
            HStack(spacing: 8) {
                Image(systemName: showMoreOptions ? "chevron.down" : "chevron.right")
                    .font(.system(size: AppTypeScale.body, weight: .semibold))
                    .foregroundStyle(.secondary)
                Text("More options").font(.system(size: AppTypeScale.title))
                Spacer(minLength: 8)
    ...
    private var promptActions: some View {
        HStack(spacing: 12) {
            ...
            Text("\(agent.systemPrompt.count)/\(AgentWriter.maxPromptCharacters)")
                .font(.system(size: AppTypeScale.body)).foregroundStyle(.tertiary)
                .monospacedDigit()
        }
    }
    ...
    private var modelState: some View {
        ...
        case .needsDownload(let path):
            HStack {
                Label("Not downloaded", systemImage: "exclamationmark.triangle.fill")
                    .foregroundStyle(.orange).font(.system(size: AppTypeScale.body))
                Spacer(minLength: 8)
        ...
        case .unavailable(let reason):
            Label(L10n.text(reason), systemImage: "wifi.slash").foregroundStyle(.orange).font(.system(size: AppTypeScale.body))
        case .noChange, .load, .lan:
    ...
    private var voiceActions: some View {
        HStack(spacing: 8) {
            ...
            if let error = previewer.error ?? clipError {
                Text(L10n.text(error)).font(.system(size: AppTypeScale.body)).foregroundStyle(.orange)
            }
            Spacer(minLength: 0)
        }
    }
