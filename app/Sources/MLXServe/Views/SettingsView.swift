...
private struct ProvidersSectionContent: View {
    ...
    private var providersBody: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let saveError {
                Text(L10n.text(saveError)).font(.system(size: AppTypeScale.body)).foregroundStyle(.red)
            }
...
private struct ProviderRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            if let problem {
                Text(L10n.text(problem)).font(.system(size: AppTypeScale.body)).foregroundStyle(.orange)
            } else if let status {
                Text(L10n.text(statusLine(status))).font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary)
            }
        }
        .padding(8)
...
private struct ProviderModelPickerSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(L10n.format("Models at %@", entry.name)).font(.system(size: AppTypeScale.title))
            TextField("Filter", text: $filter).textFieldStyle(.roundedBorder)
            if loading {
                ProgressView().frame(maxWidth: .infinity)
            } else if let error {
                Text(error).foregroundStyle(.red).font(.system(size: AppTypeScale.body))
            } else {
            ...
            HStack {
                Text("\(chosen.count) of \(ids.count) selected").font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary)
...
private struct WakePhraseSectionContent: View {
    ...
    var body: some View {
        SearchableRow(searchText: ["Wake phrase", "Hey Loki", Self.explainer]) {
            VStack(alignment: .leading, spacing: 6) {
                Text(L10n.text("Wake phrase")).font(.system(size: AppTypeScale.body, weight: .semibold))
...
private struct VoiceCloneSectionContent: View {
    ...
    private var engineBody: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(L10n.text("Voice engine")).font(.system(size: AppTypeScale.body, weight: .semibold))
    ...
    private var kokoroBody: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(L10n.text("Kokoro voice")).font(.system(size: AppTypeScale.body, weight: .semibold))
            ...
            if let e = previewer.error {
                Text(L10n.text(e)).font(.system(size: AppTypeScale.body)).foregroundStyle(.orange)
                    .fixedSize(horizontal: false, vertical: true)
            }
        }
    ...
    private var clipBody: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(L10n.text("Voice clone clip")).font(.system(size: AppTypeScale.body, weight: .semibold))
            HStack(spacing: 8) {
            ...
            Text(L10n.text(Self.explainer))
                .font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary)
            if let voiceError {
                Text(L10n.text(voiceError)).font(.system(size: AppTypeScale.body)).foregroundStyle(.red)
            }
        }
    }
