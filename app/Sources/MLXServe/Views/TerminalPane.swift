...
struct TerminalPane: View {
    ...
    private func content(_ session: TerminalSessionList.Session) -> some View {
        ...
        case .preparing:
            notice {
                ProgressView()
                Text("Starting \(session.displayName) session…").font(.system(size: AppTypeScale.title))
                Text("Boots the guest and prepares configs. First-run installs stream into the terminal once it opens.")
                    .font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary).multilineTextAlignment(.center)
            }
        case .live:
        ...
        case .failed(let message):
            notice {
                Image(systemName: "exclamationmark.triangle").font(.system(size: AppTypeScale.display)).foregroundStyle(.orange)
                Text("The \(session.displayName) session could not start").font(.system(size: AppTypeScale.title))
                Text(message)
                    .font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary)
