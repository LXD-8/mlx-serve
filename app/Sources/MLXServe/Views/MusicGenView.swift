...
struct MusicGenView: View {
    ...
    private var promptSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 8) {
                Text("Style prompt").font(.system(size: AppTypeScale.body, weight: .semibold))
                Spacer()
    ...
    private var lyricsSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 8) {
                Text("Lyrics").font(.system(size: AppTypeScale.body, weight: .semibold))
    ...
    private var coverWeightsNotice: some View {
        if task == .cover, let text = CoverWeightsFetch.notice(coverWeights) {
            VStack(alignment: .leading, spacing: 6) {
                Text(text).font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary)
                ...
                case .downloading:
                    HStack(spacing: 8) {
                        ...
                        Text(downloads.downloads[model.repo]?.percentFormatted ?? "")
                            .font(.system(size: AppTypeScale.body).monospacedDigit()).foregroundStyle(.secondary)
                        ...
                        Button("Cancel") { downloads.cancelPackFile(repoId: model.repo) }
                            .buttonStyle(.borderless).font(.system(size: AppTypeScale.body))
                    }
                default:
                    EmptyView()
                }
    ...
    private var sourceSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                Text(L10n.text(task == .cover ? "Source track" : "Source stem")).font(.system(size: AppTypeScale.body, weight: .semibold))
                Spacer()
            }
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        HStack(spacing: 8) {
                            ...
                            Text(url.lastPathComponent)
                                .font(.system(size: AppTypeScale.body)).lineLimit(1).truncationMode(.middle)
                            Spacer()
            ...
            if let err = srcError {
                Text(err).font(.system(size: AppTypeScale.body)).foregroundStyle(.orange)
            }
            if task == .cover {
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 6) {
                        Text("Cover strength").font(.system(size: AppTypeScale.body))
                        Spacer()
                        Text(String(format: "%.2f", coverStrength)).font(.system(size: AppTypeScale.body).monospacedDigit()).foregroundStyle(.secondary)
                    }
                ...
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 6) {
                        Text("Noise strength").font(.system(size: AppTypeScale.body))
                        Spacer()
                        Text(String(format: "%.2f", coverNoiseStrength)).font(.system(size: AppTypeScale.body).monospacedDigit()).foregroundStyle(.secondary)
                    }
            ...
            if task == .complete {
                Text("Add").font(.system(size: AppTypeScale.body)).padding(.top, 6)
    ...
    private var referenceSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                Text("Reference audio (optional)").font(.system(size: AppTypeScale.body, weight: .semibold))
                Spacer()
            }
    ...
    private var converting: some View {
        HStack(spacing: 8) {
            ...
            Text("Converting…").font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary)
        }
    }
    ...
    private var durationSection: some View {
        VStack(alignment: .leading, spacing: 2) {
            ...
            HStack(spacing: 6) {
                Text("Duration").font(.system(size: AppTypeScale.body, weight: .semibold))
                ...
                Text("sec · \(formattedDuration)").font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary)
                Spacer()
            }
    ...
    private func advancedCell<C: View>(_ label: String, @ViewBuilder control: () -> C) -> some View {
        ...
        VStack(alignment: .leading, spacing: 2) {
            Text(L10n.text(label)).font(.system(size: AppTypeScale.body))
            control()
        }
    }
    ...
    private var advancedBody: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if model.supportsSteps {
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 6) {
                        Text("Quality passes").font(.system(size: AppTypeScale.body))
    ...
    private var convertHint: some View {
        VStack(alignment: .leading, spacing: 5) {
            Label("Weights not found", systemImage: "wrench.and.screwdriver")
                .font(.system(size: AppTypeScale.body, weight: .semibold)).foregroundStyle(.secondary)
            ...
            Group {
                ...
                case .running(let step, let total, let message):
                    VStack(spacing: 12) {
                        ...
                        Text(message).font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary)
                    }
    ...
    private var outputFolderLink: some View {
        Button {
            ...
            Label("Open output folder in Finder", systemImage: "folder").font(.system(size: AppTypeScale.body))
        }
...
struct PromptRewriteSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Text(L10n.text(title)).font(.system(size: AppTypeScale.title))
                Spacer()
            ...
            if let error {
                Text(error).font(.system(size: AppTypeScale.body)).foregroundStyle(.red)
            } else {
