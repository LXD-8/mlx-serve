...
struct ImageGenView: View {
    ...
    private var sourceImageSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 8) {
                Text("Source image (optional)").font(.system(size: AppTypeScale.body, weight: .semibold))
                Spacer(minLength: 8)
            ...
            if let url = initImageURL {
                ...
                if effectiveEditMode {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        HStack {
                            Text("Variation strength").font(.system(size: AppTypeScale.body))
                            Spacer()
    ...
    private var resolutionSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("Resolution").font(.system(size: AppTypeScale.body, weight: .semibold))
    ...
    private func labelledSizeField(_ title: String, text: Binding<String>) -> some View {
        VStack(alignment: .leading, spacing: 2) {
            Text(L10n.text(title)).font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary)
            TextField("", text: text)
    ...
    private var advancedSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Text("Advanced (overrides Quality preset)").font(.system(size: AppTypeScale.body, weight: .semibold))
                Spacer()
                Button {
            ...
            if model.supportsGuidance {
                Divider()
                Text("Classifier-free guidance").font(.system(size: AppTypeScale.body, weight: .semibold))
                VStack(alignment: .leading, spacing: 2) {
                    Text("Guidance scale").font(.system(size: AppTypeScale.body))
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text("Negative prompt").font(.system(size: AppTypeScale.body))
            ...
            if model.condWeightCount > 0 {
                Divider()
                Text("Conditioning rebalance").font(.system(size: AppTypeScale.body, weight: .semibold))
                VStack(alignment: .leading, spacing: 2) {
                    Text("Global gain").font(.system(size: AppTypeScale.body))
