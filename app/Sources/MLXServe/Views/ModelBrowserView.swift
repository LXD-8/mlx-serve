    ...
    private func chip(_ item: ModelBrowserSection) -> some View {
        ...
        return Button {
            ...
            HStack(spacing: 5) {
                ...
                Text(L10n.text(item.title)).font(.system(size: AppTypeScale.title)).lineLimit(1)
...
private struct RecommendedTableHeader: View {
    var body: some View {
        ...
        .font(.system(size: AppTypeScale.small, weight: .semibold))
        .foregroundStyle(.secondary)
        .textCase(.uppercase)
        .padding(.horizontal, RecTableMetrics.hPad)
        .padding(.vertical, 8)
    ...
    private var tip: some View {
        VStack(alignment: .leading, spacing: 3) {
            ForEach(CapabilityTip.lines(for: pick), id: \.self) { line in
                Text(L10n.text(line)).font(.system(size: AppTypeScale.body))
            }
        }
        ...
        HStack(spacing: RecTableMetrics.spacing) {
            ...
            VStack(alignment: .trailing, spacing: 2) {
                Text(SystemMemoryInfo.preciseGB(pick.sizeGB))
                    .font(.system(size: AppTypeScale.small).monospacedDigit())
                    .foregroundStyle(.secondary)
                if let quant = pick.quantLabel {
                    Text(L10n.text(quant))
                        .font(.system(size: AppTypeScale.small))
    ...
    private var memoryCell: some View {
        VStack(alignment: .leading, spacing: 2) {
            Text(SystemMemoryInfo.preciseGB(pick.approxRAMNeededGB))
                .font(.system(size: AppTypeScale.small, weight: .medium).monospacedDigit())
            HStack(spacing: 3) {
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 1) {
                    ...
                    Text(L10n.text(subtitle)).font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary)
                }
                Spacer()
            }
    ...
    var body: some View {
        HStack(spacing: ModelBrowserMetrics.columnSpacing) {
            ...
            Text(L10n.text(model.modelSize))
                .font(.system(size: AppTypeScale.title).monospacedDigit())
                .frame(width: ModelBrowserMetrics.sizeWidth, alignment: .trailing)
