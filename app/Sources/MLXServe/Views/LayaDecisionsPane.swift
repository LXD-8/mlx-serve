...
struct LayaDecisionsPane: View {
        ...
        HSplitView {
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    Text("Laya Decisions").font(.system(size: AppTypeScale.heading).bold())
                    Text((modelPath as NSString).lastPathComponent).font(.system(size: AppTypeScale.body)).foregroundStyle(.secondary)

                    Text("State").font(.system(size: AppTypeScale.title))
                    TextEditor(text: $state).font(.system(size: AppTypeScale.body)).frame(minHeight: 80)
                        .overlay(RoundedRectangle(cornerRadius: 6).stroke(.quaternary))

                    HStack {
                        Text("Questions").font(.system(size: AppTypeScale.title))
                        Spacer()
                    ...
                    ForEach(answers, id: \.name) { a in
                        VStack(alignment: .leading, spacing: 4) {
                            HStack {
                                Text(a.name).font(.system(size: AppTypeScale.title))
                                Text(a.summary).foregroundStyle(.secondary)
                                if let c = a.confidence { Text(String(format: "confidence %.2f", c)).font(.system(size: AppTypeScale.body)).foregroundStyle(.tertiary) }
                            }
                            ForEach(a.bars, id: \.0) { label, p in
                                HStack {
                                    Text(label).frame(width: 110, alignment: .trailing).font(.system(size: AppTypeScale.body))
                                    GeometryReader { g in
                                        RoundedRectangle(cornerRadius: 3).fill(.tint).frame(width: max(2, g.size.width * p))
                                    }.frame(height: 10)
                                    Text(String(format: "%.1f%%", p * 100)).font(.system(size: AppTypeScale.body).monospacedDigit()).frame(width: 50, alignment: .trailing)
                                }
                            }
                        }
                        .padding(8)
                    ...
                    if showRaw {
                        Text("Request").font(.system(size: AppTypeScale.title))
                        codeBlock(requestJSON)
                        if !rawResponse.isEmpty {
                            Text("Response").font(.system(size: AppTypeScale.title))
    ...
    private func codeBlock(_ s: String) -> some View {
        Text(s).font(.system(size: AppTypeScale.body, design: .monospaced)).textSelection(.enabled)
    ...
    private var docs: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("What this is").font(.system(size: AppTypeScale.title))
            ...
            Text("Question types").font(.system(size: AppTypeScale.title))
            ...
            Text("API").font(.system(size: AppTypeScale.title))
